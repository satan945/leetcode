 ...
 * MyCalendar(); MyCalendar.book(10, 20); // returns true MyCalendar.book(15, 25); // returns false MyCalendar.book(20,
 * 30); // returns true Explanation: The first event can be booked. The second can't because time 15 is already booked
 * by another event. The third event can be booked, as the first event takes every time less than 20, but not including
 * 20. Note:
 * 
 * The number of calls to MyCalendar.book per test case will be at most 1000. In calls to MyCalendar.book(start, end),
 * start and end are integers in the range [0, 10^9].
 * </p>
 * 
 * @author abel created on 2017/12/1 下午6:57
 * @version $Id$
 */
public class MyCalendarTreeMap {

    private TreeMap<Integer, Integer> bookingMap;
    ...
    public boolean book(int start, int end) {
        ...
        if ((prev == null || bookingMap.get(prev) <= start) && (next == null || end <= next)) {
